...
## 2.评估框架
1. 训练框架（SFT使用LLAMA-FACTORY,参考TIMEART；强化学习trl）
2. 非训练框架
3. 消融实验
...
2. 补充verifier
构想：类似于ExpCoT那篇的思路，先进verifier，生成一些多模态证据，既引导工具，又可以提供非统计指标的视角

## 一些现状与额外任务
1. 目前全部api相关的在gpt-4o上均尝试，timeart和timetoolbench可行，主agent不稳定。更换5.4-mini在timetoolbench上需修改提示词且不稳定。
2. 调研到论文更新，去阅读借鉴。
3. 优先级：流水线方式，先跑通数据制作流程相关（3,4,6与整体优化），后在制作数据期间完成评测环境相关（2.1,2.2部分），评测期间检查完成数据集扩展（5相关）。
